...
export default function OwnerReportsSection({ stats, loading, timeframe, setTimeframe }) {
  return (
    <div>
                ...
                {stats?.monthlyData.map((data, i) => (
                  <div key={i} className="flex flex-col items-center">
                    <div
                      ...
                      style={{
                        height: `${data.revenue > 0 ? Math.max(data.revenue / 5000, 5) : 0}px`,
                      }}
                    ></div>
                    <span className="text-xs mt-1 text-gray-600">{data.month}</span>
                    <span className="text-xs text-gray-500">
                      {formatCurrency(data.revenue).replace("AOA", "").trim()}
                    </span>
                  </div>
                ))}
              </div>
